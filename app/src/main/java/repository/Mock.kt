...
class Mock {
    val ALL = MotivationConstants.Phrase.ALL
    val MORNING = MotivationConstants.Phrase.SUM
...
//       filter o contrato e com  data class, inclusive mesmo contrato para lifOf
        val filtered = mListPhrases.filter { it.category == categoryId || categoryId == ALL }
        val numberRandom: Int = Random.nextInt(filtered.size)
        return filtered[numberRandom].description
